...
| REQ-CFG-005 | (Follow-up) The customer app reads the saved config from the backend at bootstrap (an async `VenueConfigSource` with an asset fallback), so an owner edit reaches customers with no app release, not only the owner's own Settings re-read | _pending_ |
