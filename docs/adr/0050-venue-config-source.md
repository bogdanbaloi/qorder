...
compile-time constant `AppConfig.demo`. Every other provider watched it. The
...
- **Keep `AppConfig.demo` inline.** Cannot serve a second venue without a rebuild.
  It blocks both multi-venue and Settings.
...
- **Throw on an unknown venue in the provider.** A crash is the wrong UX. The port
  ...
  before routing. The provider keeps a defensive demo fallback.
