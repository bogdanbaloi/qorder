...
only the table. For many venues, the sticker must carry the venue too. The
...
- **A settable active venue.** `activeVenueIdProvider` becomes a small
  ...
  it. The app theme follows, so setting the venue reconfigures the whole app.
...
- **Fold venue handling into `MenuScreen`.** The unknown-venue case needs a
  different screen. The menu should not own venue resolution. A thin entry
